...
def analyze_movie_watching_trends(df):
    ...
    """
    #grouping by watched year and counting number of movies watched
    trendDataFrame = df.groupBy("WatchedYear").agg(count("*").alias("MoviesWatched"))
    #order results by year
    trendDataFrame = trendDataFrame.orderBy(col("WatchedYear"))

    return trendDataFrame
...
def main():
    ...
    output_file = "/workspaces/handson-7-spark-structured-api-movie-ratings-analysis-AdamLowder/outputs/movie_watching_trends.csv"
